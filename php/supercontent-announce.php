...
function deleteOldImage($id, $con){
    $sql = "SELECT images FROM announceupdate_tbl WHERE id='$id'";
...
if(isset($_POST['submit'])){
    ...
    if(isset($_FILES['image_upload']) && $_FILES['image_upload']['error'] === UPLOAD_ERR_OK){
        ...
        $target_file = $target_dir . "img" . $id . "_announce." . $image_extension;

        deleteOldImage($id, $con);

        if(move_uploaded_file($image_tmp, $target_file)){
            $target_file_sanitized = sanitizeImagePath($target_file);
            $sql = "UPDATE announceupdate_tbl SET title='$title', content='$content', images='$target_file_sanitized' WHERE id='$id'";
            if (mysqli_query($con, $sql)) {
                header("Location: ../superadmin_user-content-announce.php?update=true");
                exit();
            } else {
        ...
        $sql = "UPDATE announceupdate_tbl SET title='$title', content='$content' WHERE id='$id'";
        if (mysqli_query($con, $sql)) {
            header("Location: ../superadmin_user-content-announce.php?update=true");
            exit();
        } else {
