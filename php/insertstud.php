<?php
    ...
    if(isset($_POST['isubmit']))
    {
        ...
        if($count_id > 0)
        {
            ...
            echo '<script>
                    window.location.href = "../coordinator-add-edit-students.php?row_filled=true";
                </script>';
            exit;
        }
        ...
        if($count_email > 0)
        {
            ...
            echo '<script>
                    window.location.href = "../coordinator-add-edit-students.php?row_email=true";
                </script>';
            exit;
        }
        ...
        {
            // username and student ID number do not exist, hash the password using Bcrypt
            $hashed_password = password_hash($password, PASSWORD_BCRYPT);

            // insert the new record with the hashed password
            $sql = "INSERT INTO students_tbl (student_id_number, fname, lname, email, section, department, mobile, profile_picture, isValid) VALUES ('$student_id_number', '$fname', '$lname', '$email', '$section', '$department', '$mobile', '$profile_picture', '1')";
            $result = mysqli_query($con, $sql);

            if($result)
            {   
                // record inserted successfully, redirect to superadmin_add-edit-students.php
                header("location: ../coordinator-add-edit-students.php?success=true");
                exit;
            }
            else
            {
